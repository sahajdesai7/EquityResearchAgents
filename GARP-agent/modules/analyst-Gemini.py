...
GOOGLE_MODEL_ID = os.getenv("GOOGLE_MODEL_ID", "gemini-2.0-flash-exp")
...
if not API_KEYS:
    raise ValueError("❌ No GOOGLE_API_KEYs found in .env file. Please add at least one.")
...
def get_market_analyst():
    """
    Returns an Agent configured with Gemini using the CURRENT key.
    """
    return Agent(
        model=Gemini(id=GOOGLE_MODEL_ID, api_key=get_current_key()),
    ...
    while attempt < 3:
        try:
            ...
            if response and response.content:
                ...
                if 'error' in content:
                    error_msg = content['error']['message']
                    print(f"Error message: {error_msg}")

                    # Check for Quota/Rate Limit errors
                    if "429" in error_msg or "exhausted" in error_msg or "quota" in error_msg.lower():
                        # Extract retry delay from error message if available
                        retry_delay_match = re.search(r'Please retry in (\d+\.\d+)s', error_msg)
                        if retry_delay_match:
                            retry_delay = float(retry_delay_match.group(1))
                            print(f"⚠️ Quota/Rate Limit Hit on Key #{CURRENT_KEY_INDEX + 1}.")
                            ...
                            agent.model = Gemini(id=GOOGLE_MODEL_ID, api_key=new_key)
...
def analyze_sentiment_and_news(ticker):
    ...
    """
    print(f"\n🕵️‍♂️ Analyst Agent (Gemini): Starting 5-Step Deep Dive for {ticker}...")
